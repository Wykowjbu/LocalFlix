...
export default function GenreClient({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setGenreNotFound(false);

    Promise.all([
      fetch(`/api/movies?genre=${slug}&limit=50`).then((res) => {
        if (!res.ok) {
          if (res.status === 404) throw new Error("GENRE_NOT_FOUND");
      ...
      .then(([data]) => {
        if (data.genre) setGenreName(data.genre.name);
        const m = data.movies ? (data.movies as DbMovie[]).map(mapDbMovie) : [];
        setAllMovies(m);
        setMovies(m.slice(0, 20));
      })
      .catch((err) => {
        if (err.message === "GENRE_NOT_FOUND") setGenreNotFound(true);
        setAllMovies([]);
        setMovies([]);
      })
      .finally(() => setLoading(false));
  }, [activeProfile, slug, loadInteractions]);
  ...
  const displayTitle = genreName || slug;
      ...
      {genreNotFound ? (
        <div className="flex min-h-[60vh] flex-col items-center justify-center px-[4%] pt-[120px]">
          <h1 className="mb-4 text-[clamp(22px,2.2vw,32px)] font-medium text-white">
            Thể loại không tìm thấy
          </h1>
          <p className="text-[16px] text-[#b3b3b3]">
            Không tìm thấy thể loại &ldquo;{slug}&rdquo;. Vui lòng thử lại với thể loại khác.
          </p>
        </div>
      ) : (
        <>
          ...
          <div className="-mt-[7vw] space-y-[1vw]">
            {loading ? (
              ...
              <div className="px-[4%] py-8 text-[16px] text-[#b3b3b3]">
                Chưa có phim nào thuộc thể loại &ldquo;{displayTitle}&rdquo;.
              </div>
            )}
          </div>
        </>
